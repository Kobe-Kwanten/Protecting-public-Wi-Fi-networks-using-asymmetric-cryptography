...
static u8 *hostapd_add_eid_custom_sae_pk(u8 *pos, struct wpabuf *m) {
...
static u8 *hostapd_add_eid_fils_key_confirm(u8 *pos, const struct wpabuf *sig) {
    size_t sig_len = wpabuf_len(sig);
    /* FILS Key Confirmation element (KeyAuth) */
    *pos++ = WLAN_EID_EXTENSION;                                /* Element ID */
    *pos++ = 1 + wpabuf_len(sig);                           /* Length */
    *pos++ = WLAN_EID_EXT_FILS_KEY_CONFIRM;                     /* Extended ID */
    memcpy(pos, wpabuf_head(sig), sig_len);   /* Signature */
    pos += sig_len;
    return pos;
}

static struct wpabuf *
hostapd_gen_preauth_hash(const struct hostapd_data *hapd, const u8 *nonce, const u8 *ies, size_t ies_len, int group) {
    //hash(mac_AP || mac_station || nonce || ies)
    size_t hash_data_len = 2 * ETH_ALEN + FILS_NONCE_LEN + ies_len;
    struct wpabuf *hash_data = wpabuf_alloc(hash_data_len);
    wpabuf_put_data(hash_data, hapd->own_addr, ETH_ALEN);
    wpabuf_put_data(hash_data, hapd->probe_sta_address, ETH_ALEN);
    wpabuf_put_data(hash_data, nonce, FILS_NONCE_LEN);
    wpabuf_put_data(hash_data, ies, ies_len);

    //wpa_hexdump_buf_key(MSG_DEBUG, "Preauth-Attacks: Data to be signed for KeyAuth",
    //                    hash_data);
    size_t hash_len = sae_group_2_hash_len(group);
    u8 hash[SAE_MAX_HASH_LEN];
    if (sae_hash(hash_len, wpabuf_head(hash_data), wpabuf_len(hash_data), hash) < 0) {
        wpabuf_free(hash_data);
        return NULL;
    }

    wpabuf_free(hash_data);
    // wpa_hexdump(MSG_DEBUG, "Preauth-Attacks: hash(data to be signed)",
    //            hash, hash_len);
    struct wpabuf *res = wpabuf_alloc(hash_len);
    wpabuf_put_data(res, hash, hash_len);
    return res;
}

static u8 *hostapd_add_preauth_attack_eids(struct hostapd_data *hapd, u8 *pos, u8 *ies) {
    u8 *original_pos = pos;
    struct sae_password_entry *password_entry = hapd->conf->sae_passwords;
    const u8 *nonce = hapd->probe_req_nonce;
    if (nonce && password_entry) {
        wpa_hexdump(MSG_DEBUG, "Preauth_Attacks: Received probe request with nonce",
                    nonce, FILS_NONCE_LEN);

        struct wpabuf *pubkey = password_entry->pk->pubkey;
        pos = hostapd_eid_fils_pub_key(pos, pubkey);

        struct wpabuf *m = password_entry->pk->m;
        pos = hostapd_add_eid_custom_sae_pk(pos, m);

        /* Generate hash for signature */
        int group = password_entry->pk->group;
        size_t ies_len = pos - ies;
        struct wpabuf *hash = hostapd_gen_preauth_hash(hapd, nonce, ies, ies_len, group);

        /* Sign hashed data */
        struct wpabuf *sig =
                crypto_ec_key_sign(password_entry->pk->key, wpabuf_head(hash), wpabuf_len(hash));
        if (!sig)
            return original_pos;
        wpa_hexdump_buf(MSG_DEBUG, "Preauth-Attacks: KeyAuth = Sig_AP()", sig);
        pos = hostapd_add_eid_fils_key_confirm(pos, sig);
        wpabuf_free(sig);
    }
    return pos;
}
...
#define MAX_PROBERESP_LEN 768
    ...
    *pos++ = hapd->conf->ssid.ssid_len;
    ...
    pos = hostapd_add_preauth_attack_eids(hapd, pos, resp->u.probe_resp.variable);
...
/**
 * Wordt gecalled door iee802_11_mgmt om een probe request af te handelen.
 */
...
#ifdef CONFIG_PREAUTH_ATTACKS
    hapd->probe_req_nonce = elems.fils_nonce;
    memcpy(hapd->probe_sta_address, mgmt->sa, ETH_ALEN);
#endif /* CONFIG_PREAUTH_ATTACKS*/
...
#ifdef CONFIG_PREAUTH_ATTACKS
    struct sae_password_entry *password_entry = hapd->conf->sae_passwords;
    tailpos = hostapd_eid_fils_pub_key(tailpos, password_entry->pk->pubkey);
    tailpos = hostapd_eid_custom_sae_pk(tailpos, password_entry->pk->m);
#endif /* CONFIG_PREAUTH_ATTACKS */
...
#ifdef CONFIG_PREAUTH_ATTACKS
    params->sae_pk_key = hapd->conf->sae_passwords->pk->key;
    params->der = password_entry->pk->der;
    params->der_len = password_entry->pk->der_len;
#endif /* CONFIG_PREAUTH_ATTACKS */
